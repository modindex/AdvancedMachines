...
public class MachineControllerDefault implements MachineController, ItemObservable.IObserver, FluidObservable.IObserver, IEnergyObservable.IObserver, 
	...
	public MachineControllerDefault(ItemHandlerSeparated inventory, FluidHandler fluidtank, IEnergyStorageAdvanced energy, RecipeManager recipemanager, MachineTile te) {
	...
	public MachineControllerDefault(StorageCombined storage, RecipeManager recipemanager, MachineTile te) {
		inventory = storage;
		fluidtank = storage;
		energy = storage;
		this.recipemanager = recipemanager;
	...
	protected void preProcess() {
		for (SubController obvserver : subcontrollers) {
			if (obvserver.preProcess((MachineController)this)) { wake(); }
		}
	}
	...
	protected void postProcess() {
		for (SubController obvserver : subcontrollers) {
			if (obvserver.postProcess((MachineController)this)) { wake(); }
		}
	}
